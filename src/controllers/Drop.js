...
class DropController extends Controller {
  ...
  trim = [
    ...
    body('end')
      ...
      const {body: {tag, start, end}} = req;
  		const dropService = new DropService(this.response.bind(this, res));
  ...
  validate = [
  	body('recording')
      .notEmpty()
  		.withMessage('must be a valid recording.'),

  	this.action(async (req, res, next) => {
      const {body: {recording}} = req;
  		const dropService = new DropService(this.response.bind(this, res));
  		const response = await dropService.validate(recording);
      this.response(res, response.code, response.data, response.message);
  		next();
  	})
  ];
  ...
  waveform = [
      ...
      const {query: {tag, bars}} = req;
  		const dropService = new DropService(this.response.bind(this, res));
  		const response = await dropService.validate(tag, bars);
      this.response(res, response.code, response.data, response.message);
  		next();
  	})
  ];
  ...
  create = [
  	...
  	this.action(async (req, res, next) => {
      const {body: {user_id, tag, caption, category}} = req;
  		const dropService = new DropService();
      // Get user_id from firebase
  		const response = await dropService.create(user_id, tag, caption, category);
      this.response(res, response.code, response.data, response.message);
  		next();
  	})
  ];
