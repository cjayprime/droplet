...
setTimeout(async () => {
  if (process.env.NODE_ENV === 'development') {
    ...
    // const response = await request.post('/validate').send({ recording, user_id: 'CQT6WGHo5FYoY60zzjB3eNcAFA73', source: 'recording' });
    // const response = await request.put('/trim').send({ start: 35, end: 60, tag: '00ba23bd-17e1-4001-aac0-87603c8ea18f' });
    // const response = await request.get('/download?tag=00ba23bd-17e1-4001-aac0-87603c8ea18f&isTrimmed=true').send();
    // const response = await request.get('/waveform?tag=0c5ffe6f-5471-4612-bfe4-d6c93518a640&bars=10').send();
    // const response = await request.get('/categories').send();
    // const response = await request.post('/create').send({ user_id: '0sbfQ2A1DCU79XbPiuYpCy4NhAx2', tag: 'e73a0907-60e7-47ed-b278-917f401f93dc', caption: 'Done', category: 'music', isTrimmed: false });
    // const response = await request.get('/analytics').send();
    // const response = await request.post('/listen').send({ user_id: '0sbfQ2A1DCU79XbPiuYpCy4NhAx2', drop_id: 1 });
    // const response = await request.post('/like').send({ user_id: 1, drop_id: 1 });
    // const response = await request.get('/drops?offset=0&limit=10&category=comedy&category=convo').send();
    // const response = await request.get('/drops/user/0sbfQ2A1DCU79XbPiuYpCy4NhAx2?offset=0&limit=10&category=comedy&category=convo').send();
    // const response = await request.get('/drops/1'/*/drops/e73a0907-60e7-47ed-b278-917f401f93dc*/).send();
