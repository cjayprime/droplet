...
const application = new Application({
  ...
  middlewares: [
    ...
    function devlog(req, _, next) {
      console.log('\n\n-- INCOMING REQUEST', req.method, req.originalUrl, req.body, req.query);
      next();
    },
    ...
    expressJWT({ secret: Configuration.privateKey(), algorithms: ['RS256'], requestProperty: 'account' })
      .unless((req) => {
        ...
        const url = originalUrl[0].split(':');
        return Controllers.public.some(ctrl => url[0] === ctrl.path && req.method === ctrl.method);
      }),
...
setTimeout(async () => {
  if (process.env.NODE_ENV === 'development') {
    ...
    // const response = await request.post('/validate').send({recording}).set(headers);
